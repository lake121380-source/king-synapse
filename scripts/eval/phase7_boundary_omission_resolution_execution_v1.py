#!/usr/bin/env python3
"""Phase 7.3.3-D1-B4 Boundary Omission Resolution v1."""
...
PROTOCOL=CONFIG/'phase7_3_3_d_boundary_omission_resolution_protocol_v1.json'
POLICY=CONFIG/'phase7_3_3_d_boundary_omission_resolution_execution_policy_v1.json'
PROMPT=CONFIG/'phase7_3_3_d_boundary_omission_resolution_prompt_v1.md'
WORKLIST=DATA/'phase7_3_3_d_boundary_omission_resolution_packet_v1.json'
FIXTURES=REPORTS/'phase7_3_3_d_boundary_omission_resolution_contract_fixtures_v1.json'
MANIFEST=REPORTS/'phase7_3_3_d_boundary_omission_resolution_execution_manifest_v1.json'
ATTEMPT_LOG=REPORTS/'phase7_3_3_d_boundary_omission_resolution_attempts_v1.jsonl'
CASE_DIR=REPORTS/'phase7_3_3_d_boundary_omission_resolution_cases_v1'
RESULT=REPORTS/'phase7_3_3_d_boundary_omission_resolution_execution_result_v1.json'
SUBMISSION=REPORTS/'phase7_3_3_d_boundary_omission_resolution_submission_v1.json'
CORRECTION=REPORTS/'phase7_3_3_d_boundary_correction_worklist_v1.json'
READINESS=REPORTS/'phase7_3_3_d1_reference_construction_readiness_v9.json'
...
def protocol()->dict[str,Any]:
    return {
      'schema_version':1,
      'protocol_id':'phase7.3.3-d1-b4-boundary-omission-resolution-v1',
...
def policy()->dict[str,Any]:
    return {'schema_version':1,'policy_id':'phase7.3.3-d1-b4-boundary-omission-resolution-execution-policy-v1','authoritative_result_policy':{'first_provider_content_authoritative':True,'invalid_json_schema_or_semantics_authoritative_negative':True,'semantic_retry':False,'transport_failure_before_content_resume_same_manifest':True},'execution_controls':{'case_isolation':True,'model':MODEL,'temperature':TEMP,'top_p':TOP_P,'max_tokens':MAX_TOKENS,'response_format':RESP},'data_handling':{'credential_env_name':CREDENTIAL_ENV,'raw_provider_response_stored':False,'envelope_hash_recorded':True,'content_hash_recorded':True,'held_out_loaded':False},'prohibitions':['no_boundary_repair','no_coverage_qa_rerun','no_gold_freeze','no_support_review']}

def prompt()->str:
    return '''# Phase 7.3.3-D1-B4 Boundary Omission Resolution Prompt v1
...
def build_worklist()->dict[str,Any]:
    ...
    return {'schema_version':1,'worklist_id':'phase7.3.3-d1-b4-boundary-omission-resolution-packet-v1','status':'frozen_input_worklist','case_count':4,'gap_count':4,'case_isolation':True,'held_out_accessed':False,'source_lineage':{'b3_worklist_sha256':sha(B3_WORKLIST),'b3_submission_sha256':sha(B3_SUBMISSION),'b3_readiness_sha256':sha(B3_READINESS),'b2_packet_sha256':sha(B2_PACKET)},'cases':cases}
...
def fixtures()->dict[str,Any]:
    ...
    return {'schema_version':1,'report_id':'phase7.3.3-d1-b4-boundary-omission-resolution-contract-fixtures-v1','fixture_count':len(r),'fixtures_passed':sum(x['fixture_passed'] for x in r),'all_fixtures_passed':all(x['fixture_passed'] for x in r),'provider_called':False,'held_out_accessed':False,'results':r}
...
def expected_manifest()->dict[str,Any]:
    ...
    return {'schema_version':1,'manifest_id':'phase7.3.3-d1-b4-boundary-omission-resolution-execution-v1','status':'frozen_not_started','provider':'api.gpt.ge','provider_base_url':BASE_URL,'model_requested':MODEL,'temperature':TEMP,'top_p':TOP_P,'max_tokens':MAX_TOKENS,'response_format':RESP,'credential_env_name':CREDENTIAL_ENV,'adapter_sha256':sha(Path(__file__)),'protocol_sha256':sha(PROTOCOL),'execution_policy_sha256':sha(POLICY),'prompt_sha256':sha(PROMPT),'worklist_sha256':sha(WORKLIST),'contract_fixtures_sha256':sha(FIXTURES),'b3_worklist_sha256':sha(B3_WORKLIST),'b3_submission_sha256':sha(B3_SUBMISSION),'case_count':load(WORKLIST)['case_count'],'gap_count':load(WORKLIST)['gap_count'],'case_isolation':True,'first_provider_content_authoritative':True,'transport_resume_same_manifest_only':True,'semantic_retry_authorized':False,'raw_provider_responses_stored':False,'held_out_accessed':False}
...
def execute()->int:
    ...
    append_event({'event_type':'d1_b4_execution_invocation','manifest_sha256':mh,'status':'started_or_resumed','response_received':False,'authoritative_result':False},ATTEMPT_LOG)
    results=[]
    for case in w['cases']:
