#!/usr/bin/env python3
"""Run Manifest v2 of the frozen Phase 7.3.3-A Atomic Judge controls.

The immutable v2 manifest selects every artifact, model parameter, and exactly
one credential environment variable. Credentials are never persisted. Gold Claim labels, expected Candidate labels, design cases,
...
CHECKPOINT = ROOT / "target/phase7/phase7_3_3_a_atomic_judge_checkpoint_v2.json"
...
def main() -> int:
    ...
    attempt_number = next_attempt_number(manifest_hash)
    ...
    append_event({
        ...
        "occurrence_time_status": "recorded_by_v2_adapter",
        "status": "in_progress",
    ...
    for index, case in enumerate(cases, start=1):
        ...
        try:
            ...
            append_event({
                ...
                "failure_type": f"http_{error.code}",
                "case_id": case_id,
                "response_received": attempt_returned_count > 0,
        ...
        results[case_id] = row
        append_event({
            "event_type": "attempt_experimental_output_received",
            "manifest_sha256": manifest_hash,
            "attempt_number": attempt_number,
            "attempt_occurred_at": now(),
            "occurrence_time_status": "recorded_by_v2_adapter",
