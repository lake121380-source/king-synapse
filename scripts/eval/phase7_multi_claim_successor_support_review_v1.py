#!/usr/bin/env python3
"""Frozen independent Support Review for the identifiable multi-claim successor frame."""
...
STATE=D/'phase7_3_3_d_support_stage_state_v51.json'; READY=R/'phase7_3_3_d1_reference_construction_readiness_v62.json'; PACKET_RECEIPT=R/'phase7_3_3_d_multi_claim_successor_support_packet_construction_receipt_v1.json'; REF_SEAL=R/'phase7_3_3_d_multi_claim_successor_type_metadata_reference_candidate_seal_v1.json'
PROTOCOL=C/'phase7_3_3_d_multi_claim_successor_support_review_protocol_v1.json'; SCHEMA=C/'phase7_3_3_d_multi_claim_successor_support_review_schema_v1.json'; PROMPT=C/'phase7_3_3_d_multi_claim_successor_support_reviewer_prompt_v1.md'; POLICY=C/'phase7_3_3_d_multi_claim_successor_support_review_execution_policy_v1.json'; FIXTURES=R/'phase7_3_3_d_multi_claim_successor_support_review_contract_fixtures_v1.json'
JOINT_REPORT=R/'phase7_3_3_d_multi_claim_successor_support_review_execution_report_v1.json'; JOINT_OUTCOME=R/'phase7_3_3_d_multi_claim_successor_support_review_execution_outcome_v1.json'; JOINT_RECEIPT=R/'phase7_3_3_d_multi_claim_successor_support_review_execution_receipt_v1.json'; STATE_OUT=D/'phase7_3_3_d_support_stage_state_v52.json'; READY_OUT=R/'phase7_3_3_d1_reference_construction_readiness_v63.json'
CURRENT='execute_multi_claim_successor_independent_support_review_v1'; NEXT_OK='compute_multi_claim_successor_support_agreement_v1'; NEXT_NEG='design_new_multi_claim_successor_support_review_version_if_research_continues'
...
TOP={'decisions'}; DKEYS={'claim_index','support_label','cited_evidence_indices','reason_codes','support_rationale','annotation_confidence'}
EXPECTED={STATE:'fbdcfc21ed4695ff31cbd77d5d28ca7a3996695aa78d1433672003a5d0afc395',READY:'c924c4d006e2c41913b6e94884a2d79e515388863f986e92eb2432c141c8139c',PACKET_RECEIPT:'ad196037f6cb8c8291f2fa2e7f73524bfd8a35ee0ca15daf68eddc3811aa0fec',REF_SEAL:'d2c0d394cd11d47d5eed3352d1d74bcc16178e91049038968ccfc3c31bee1fd9'}
...
def manifest(r):return R/f'phase7_3_3_d_multi_claim_successor_support_reviewer_{r}_execution_manifest_v1.json'
def attempts(r):return R/f'phase7_3_3_d_multi_claim_successor_support_reviewer_{r}_execution_attempts_v1.jsonl'
def case_dir(r):return R/f'phase7_3_3_d_multi_claim_successor_support_reviewer_{r}_cases_v1'
def checkpoint(r,c):return case_dir(r)/f'{c}.json'
def submission(r):return R/f'phase7_3_3_d_multi_claim_successor_support_reviewer_{r}_completed_submission_v1.json'
def result(r):return R/f'phase7_3_3_d_multi_claim_successor_support_reviewer_{r}_execution_result_v1.json'
def negative(r):return R/f'phase7_3_3_d_multi_claim_successor_support_reviewer_{r}_negative_result_v1.json'
...
def protocol_doc():return {'schema_version':1,'protocol_id':'phase7.3.3-d-multi-claim-successor-independent-support-review-v1','stage':'Multi-claim Successor Reference Construction / Support Review','research_object':'independent evidential support classification for sealed multi-claim reference candidates','entry_gate':{'current_gate':CURRENT,'case_count':40,'claim_count':240,'support_packets_frozen':True,'reference_candidate_sealed':True,'support_gold_created':False,'confirmatory_dataset_opened':False,'runtime_integration_authorized':False},'reviewers':{r:{'model_requested':v['model'],'independent':True} for r,v in REVIEWERS.items()},'representation':{'type':'operation_index_based','model_outputs_claim_index_not_reference_claim_id':True,'model_outputs_evidence_indices_not_evidence_ids':True,'adapter_reconstructs_immutable_ids':True,'claim_indices_one_based':True,'evidence_indices_one_based':True,'claim_mutation_forbidden':True},'independence':{'reviewer_packet_isolation':True,'other_reviewer_submission_visible':False,'old_gold_visible':False,'arm_outputs_visible':False,'boundary_adjudication_rationales_visible':False,'external_tools_enabled':False,'web_access_enabled':False,'memory_enabled':False},'output_contract':{'required_top_level_keys':sorted(TOP),'required_decision_keys':sorted(DKEYS),'one_decision_per_reference_claim':True,'claim_order_must_match_packet':True,'support_label_enum':sorted(LABELS),'reason_code_enum':sorted(REASONS),'confidence_enum':sorted(CONF),'same_case_citations_only':True,'citations_may_be_empty':True,'reason_codes_may_be_empty':True,'duplicates_forbidden':True,'nonempty_rationale':True},'provider_contract':{'provider':'api.gpt.ge','base_url':BASE,'credential_env_name':CRED,'temperature':TEMP,'top_p':TOP_P,'max_tokens':MAX_TOKENS,'response_format':RESPONSE_FORMAT,'first_provider_content_authoritative':True,'envelope_and_content_hashes_recorded':True,'raw_provider_response_stored':False},'failure_policy':{'pre_content_transport_failure_resumable_same_manifest':True,'post_content_failure_authoritative_negative':True,'post_content_same_version_retry_forbidden':True,'other_reviewer_may_continue':True,'agreement_requires_two_completed_submissions':True},'completion_gate':{'completed_case_count':40,'support_decision_count':240,'both_reviewers_required_for_agreement':True}}
def schema_doc():return {'$schema':'https://json-schema.org/draft/2020-12/schema','$id':'phase7.3.3-d-multi-claim-successor-support-review-schema-v1','type':'object','additionalProperties':False,'required':['decisions'],'properties':{'decisions':{'type':'array','items':{'type':'object','additionalProperties':False,'required':sorted(DKEYS),'properties':{'claim_index':{'type':'integer','minimum':1},'support_label':{'type':'string','enum':sorted(LABELS)},'cited_evidence_indices':{'type':'array','items':{'type':'integer','minimum':1},'uniqueItems':True},'reason_codes':{'type':'array','items':{'type':'string','enum':sorted(REASONS)},'uniqueItems':True},'support_rationale':{'type':'string','minLength':1},'annotation_confidence':{'type':'string','enum':sorted(CONF)}}}}}}
def prompt_doc():return '''# Phase 7.3.3-D Multi-claim Successor Independent Support Reviewer Prompt v1
...
def normalize(case,obj):
 ...
 out=[];evidence=case['evidence_bundle']
 for claim,d in zip(case['claims'],ds,strict=True):
  ...
  cites=d['cited_evidence_indices']
  if not isinstance(cites,list):raise ReviewFailure('cited_evidence_indices_not_array','level_1_schema','schema_failure')
  if any(isinstance(x,bool) or not isinstance(x,int) for x in cites):raise ReviewFailure('cited_evidence_index_not_integer','level_1_schema','index_failure')
  if len(cites)!=len(set(cites)):raise ReviewFailure('duplicate_cited_evidence_index','level_2_support_contract','provenance_failure')
  if any(x<1 or x>len(evidence) for x in cites):raise ReviewFailure('cited_evidence_index_out_of_range','level_2_support_contract','provenance_failure')
  ...
  out.append({'case_id':case['case_id'],'reference_claim_id':claim['reference_claim_id'],'claim_index':i,'support_label':label,'cited_evidence_indices':cites,'cited_evidence_ids':[evidence[x-1]['evidence_id'] for x in cites],'reason_codes':reasons,'support_rationale':rat.strip(),'annotation_confidence':conf})
 return out
def fixture_doc():
 case={'case_id':'fixture','evidence_bundle':[{'evidence_id':'e1','content':'alpha'},{'evidence_id':'e2','content':'beta'}],'claims':[{'reference_claim_id':'c1','claim_index':1},{'reference_claim_id':'c2','claim_index':2}]};d1={'claim_index':1,'support_label':'supported','cited_evidence_indices':[1],'reason_codes':['direct_evidence_match'],'support_rationale':'Direct.','annotation_confidence':'high'};d2={'claim_index':2,'support_label':'unsupported','cited_evidence_indices':[],'reason_codes':['insufficient_evidence'],'support_rationale':'Absent.','annotation_confidence':'medium'};v={'decisions':[d1,d2]}
 fs=[('valid',v,1),('empty_reasons',{'decisions':[{**d1,'reason_codes':[]},d2]},1),('extra_top',{**v,'case_id':'x'},0),('missing',{'decisions':[d1]},0),('reorder',{'decisions':[d2,d1]},0),('bool_index',{'decisions':[{**d1,'claim_index':True},d2]},0),('bad_label',{'decisions':[{**d1,'support_label':'not_supported'},d2]},0),('bad_cite',{'decisions':[{**d1,'cited_evidence_indices':[3]},d2]},0),('dup_cite',{'decisions':[{**d1,'cited_evidence_indices':[1,1]},d2]},0),('bad_reason',{'decisions':[{**d1,'reason_codes':['invented']},d2]},0),('dup_reason',{'decisions':[{**d1,'reason_codes':['scope_preserved']*2},d2]},0),('blank',{'decisions':[{**d1,'support_rationale':' '},d2]},0),('bad_conf',{'decisions':[{**d1,'annotation_confidence':'certain'},d2]},0),('extra_key',{'decisions':[{**d1,'reference_claim_id':'forbidden'},d2]},0),('cites_scalar',{'decisions':[{**d1,'cited_evidence_indices':1},d2]},0)]
 rs=[]
 for n,x,w in fs:
  ok=1;err=None
 ...
 for n,q,x,w in [('identity_exact_gpt','gpt-4.1','gpt-4.1',1),('identity_prefixed_gpt','gpt-4.1','openai/gpt-4.1',1),('identity_snapshot_gpt','gpt-4.1','openai/gpt-4.1-2025-04-14',1),('identity_exact_qwen','qwen3.5-plus','qwen3.5-plus',1),('identity_prefixed_qwen','qwen3.5-plus','qwen/qwen3.5-plus',1),('identity_wrong','qwen3.5-plus','qwen/qwen-max',0)]:
  ...
  rs.append({'fixture':n,'expected_pass':bool(w),'observed_pass':bool(ok),'fixture_passed':ok==w,'observed_error':err})
 return {'schema_version':1,'report_id':'phase7.3.3-d-multi-claim-successor-support-review-contract-fixtures-v1','fixture_count':len(rs),'fixtures_passed':sum(x['fixture_passed'] for x in rs),'all_fixtures_passed':all(x['fixture_passed'] for x in rs),'provider_called':False,'results':rs}
...
def expected_manifest(r):
 ...
 return {'schema_version':1,'manifest_id':f'phase7.3.3-d-multi-claim-successor-support-reviewer-{r}-execution-manifest-v1','status':'frozen_not_started','reviewer_id':r,'reviewer_label':v['label'],'reviewer_type':'ai_model','provider':'api.gpt.ge','provider_base_url':BASE,'model_requested':v['model'],'temperature':TEMP,'top_p':TOP_P,'max_tokens':MAX_TOKENS,'response_format':RESPONSE_FORMAT,'timeout_seconds':TIMEOUT,'credential_env_name':CRED,'adapter_sha256':sha(SELF),'protocol_sha256':sha(PROTOCOL),'schema_sha256':sha(SCHEMA),'prompt_sha256':sha(PROMPT),'execution_policy_sha256':sha(POLICY),'contract_fixtures_sha256':sha(FIXTURES),'support_stage_state_v51_sha256':sha(STATE),'readiness_v62_sha256':sha(READY),'packet_receipt_sha256':sha(PACKET_RECEIPT),'reference_seal_sha256':sha(REF_SEAL),'review_packet_sha256':sha(packet(r)),'submission_template_sha256':sha(template(r)),'packet_id':p['packet_id'],'case_count':p['case_count'],'claim_count':p['claim_count'],'evidence_item_count':p['evidence_item_count'],'representation':'operation_index_based','id_reconstruction':'deterministic_adapter','case_isolation':True,'other_reviewer_visible':False,'old_gold_visible':False,'arm_outputs_visible':False,'boundary_adjudication_rationales_visible':False,'external_tools_enabled':False,'web_access_enabled':False,'memory_enabled':False,'raw_provider_responses_stored':False,'first_provider_content_authoritative':True,'pre_content_transport_resume_allowed':True,'post_content_same_version_retry_allowed':False,'model_identity_policy':'provider namespace and revision suffix only within requested family','confirmatory_dataset_opened':False,'runtime_integration_authorized':False}
def prepare():
 g=preflight()
...
def finalize():
 ...
 report={'schema_version':1,'report_id':'phase7.3.3-d-multi-claim-successor-support-review-execution-report-v1','status':'PASS' if both else 'AUTHORITATIVE_NEGATIVE_RESULT','both_reviewers_completed':both,'agreement_authorized':both,'reviewer_outcomes':terms,'reviewer_artifacts':arts,'case_count_per_completed_reviewer':40,'claim_count_per_completed_reviewer':240,'support_gold_created':False,'confirmatory_dataset_opened':False,'runtime_integration_authorized':False,'next_authorized_stage':nxt};rh=once(JOINT_REPORT,report);out={'schema_version':1,'outcome_id':'phase7.3.3-d-multi-claim-successor-support-review-execution-outcome-v1','status':report['status'],'execution_report_sha256':rh,'both_reviewers_completed':both,'agreement_authorized':both,'support_reference_candidate_created':False,'support_gold_created':False,'confirmatory_dataset_opened':False,'runtime_integration_authorized':False,'next_authorized_stage':nxt};oh=once(JOINT_OUTCOME,out)
 s=copy.deepcopy(load(STATE));rd=copy.deepcopy(load(READY));line={'multi_claim_successor_support_review_protocol_v1_sha256':sha(PROTOCOL),'multi_claim_successor_support_review_schema_v1_sha256':sha(SCHEMA),'multi_claim_successor_support_reviewer_prompt_v1_sha256':sha(PROMPT),'multi_claim_successor_support_review_execution_policy_v1_sha256':sha(POLICY),'multi_claim_successor_support_review_contract_fixtures_v1_sha256':sha(FIXTURES),'multi_claim_successor_support_reviewer_a_manifest_v1_sha256':sha(manifest('a')),'multi_claim_successor_support_reviewer_b_manifest_v1_sha256':sha(manifest('b')),'multi_claim_successor_support_review_execution_report_v1_sha256':rh,'multi_claim_successor_support_review_execution_outcome_v1_sha256':oh}
