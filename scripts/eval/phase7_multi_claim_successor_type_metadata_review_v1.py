#!/usr/bin/env python3
"""Freeze and execute independent Type/Metadata Review v1."""
...
STATE_IN=PATTERN/'phase7_3_3_d_support_stage_state_v37.json'; READY_IN=REPORTS/'phase7_3_3_d1_reference_construction_readiness_v48.json'
...
SCOPE=CONFIG/'phase7_3_3_d_multi_claim_successor_type_metadata_scope_decision_v1.json'; PROTOCOL=CONFIG/'phase7_3_3_d_multi_claim_successor_type_metadata_review_protocol_v1.json'
SCHEMA=CONFIG/'phase7_3_3_d_multi_claim_successor_type_metadata_review_output_schema_v1.json'; POLICY=CONFIG/'phase7_3_3_d_multi_claim_successor_type_metadata_review_execution_policy_v1.json'
PROMPT=CONFIG/'phase7_3_3_d_multi_claim_successor_type_metadata_reviewer_prompt_v1.md'; WORKLIST=PATTERN/'phase7_3_3_d_multi_claim_successor_type_metadata_blind_worklist_v1.json'
FIXTURES=REPORTS/'phase7_3_3_d_multi_claim_successor_type_metadata_review_contract_fixtures_v1.json'; PREP_MANIFEST=REPORTS/'phase7_3_3_d_multi_claim_successor_type_metadata_review_prepare_manifest_v1.json'
PREP_OUTCOME=REPORTS/'phase7_3_3_d_multi_claim_successor_type_metadata_review_prepare_outcome_v1.json'; PREP_RECEIPT=REPORTS/'phase7_3_3_d_multi_claim_successor_type_metadata_review_prepare_receipt_v1.json'
STATE_PREP=PATTERN/'phase7_3_3_d_support_stage_state_v38.json'; READY_PREP=REPORTS/'phase7_3_3_d1_reference_construction_readiness_v49.json'
ATTEMPT_LOG=REPORTS/'phase7_3_3_d_multi_claim_successor_type_metadata_reviewer_attempts_v1.jsonl'
CURRENT='construct_multi_claim_successor_boundary_type_metadata_review_v1'; EXEC_A='execute_multi_claim_successor_independent_type_metadata_review_a_v1'; EXEC_B='execute_multi_claim_successor_independent_type_metadata_review_b_v1'; AGREEMENT='construct_multi_claim_successor_type_metadata_agreement_v1'
...
EXPECTED={STATE_IN:'f1216f362dbcd4bea33f91bce5c264cd896a571e42a753f92c7dc82605df8ca3',READY_IN:'b77b5c292d0a77f6e0e23658ce344665eff11063d2ab70fb1490f234ece52721',REFERENCE:'e93abf01b54a6276a4f5ae2e370fd891548218a96022129a20030280e034b2ac',SOURCE:'13656be468d8c48c36967c689de4d0fdad09cd7f9ba9efe619682863659a2405'}
...
def fixtures():
 ...
 return {'schema_version':1,'fixture_id':'phase7.3.3-d-multi-claim-successor-type-metadata-review-contract-fixtures-v1','passed':sum(x['status']=='PASS' for x in rows),'total':len(rows),'status':'PASS' if all(x['status']=='PASS' for x in rows) else 'FAIL','results':rows}
def manifest_path(r):return REPORTS/f'phase7_3_3_d_multi_claim_successor_type_metadata_reviewer_{r}_execution_manifest_v1.json'
def checkpoint(r,c):return REPORTS/'phase7_3_3_d_multi_claim_successor_type_metadata_reviewer_cases_v1'/r/f'{c}.json'
def submission(r):return REPORTS/f'phase7_3_3_d_multi_claim_successor_type_metadata_reviewer_{r}_submission_v1.json'
def result_path(r):return REPORTS/f'phase7_3_3_d_multi_claim_successor_type_metadata_reviewer_{r}_execution_result_v1.json'
def outcome_path(r):return REPORTS/f'phase7_3_3_d_multi_claim_successor_type_metadata_reviewer_{r}_execution_outcome_v1.json'
def receipt_path(r):return REPORTS/f'phase7_3_3_d_multi_claim_successor_type_metadata_reviewer_{r}_execution_receipt_v1.json'
def negative_path(r):return REPORTS/f'phase7_3_3_d_multi_claim_successor_type_metadata_reviewer_{r}_negative_result_v1.json'
def manifest_doc(r):
 return {'schema_version':1,'manifest_id':f'phase7.3.3-d-multi-claim-successor-type-metadata-reviewer-{r}-execution-manifest-v1','status':'frozen_not_started','reviewer':r,'provider':'api.gpt.ge','provider_base_url':BASE_URL,'model_requested':REVIEWERS[r],'temperature':0,'top_p':1,'max_tokens':1600,'response_format':{'type':'json_object'},'credential_env_name':CRED,'adapter_sha256':sha(SELF),'scope_decision_sha256':sha(SCOPE),'protocol_sha256':sha(PROTOCOL),'schema_sha256':sha(SCHEMA),'policy_sha256':sha(POLICY),'prompt_sha256':sha(PROMPT),'worklist_sha256':sha(WORKLIST),'fixtures_sha256':sha(FIXTURES),'boundary_reference_candidate_sha256':sha(REFERENCE),'case_count':40,'claim_count':240,'case_isolation':True,'first_provider_content_authoritative':True,'semantic_retry_authorized':False,'raw_provider_content_stored':False,'evidence_visible':False,'support_labels_visible':False,'other_reviewer_visible':False,'boundary_mutation_authorized':False,'confirmatory_dataset_opened':False,'runtime_integration_authorized':False}
def preflight():
...
def prepare_run():
 ...
 s.update({'schema_version':38,'state_id':'phase7.3.3-d-support-stage-state-v38','status':'multi_claim_successor_type_metadata_review_v1_prepared','next_authorized_stage':EXEC_A,'successor_type_metadata_review_v1_frozen':True,'successor_type_metadata_reviewer_a_manifest_frozen':True,'successor_type_metadata_reviewer_b_manifest_frozen':True,'successor_type_metadata_provider_called':False,'confirmatory_opening_authorized':False,'confirmatory_dataset_opened':False,'runtime_integration_authorized':False})
 r.update({'schema_version':49,'readiness_id':'phase7.3.3-d1-reference-construction-readiness-v49','status':'multi_claim_successor_type_metadata_review_v1_prepared','next_authorized_stage':EXEC_A,'successor_type_metadata_review_v1_frozen':True,'successor_type_metadata_reviewer_a_manifest_frozen':True,'successor_type_metadata_reviewer_b_manifest_frozen':True,'successor_type_metadata_provider_called':False,'confirmatory_opening_authorized':False,'confirmatory_dataset_opened':False,'runtime_integration_authorized':False})
 ssha=write_once(STATE_PREP,s);rsha=write_once(READY_PREP,r)
 outcome={'schema_version':1,'outcome_id':'phase7.3.3-d-multi-claim-successor-type-metadata-review-prepare-outcome-v1','status':'PASS','case_count':40,'claim_count':240,'provider_called':False,'next_authorized_stage':EXEC_A};write_once(PREP_OUTCOME,outcome)
 receipt={'schema_version':1,'receipt_id':'phase7.3.3-d-multi-claim-successor-type-metadata-review-prepare-receipt-v1','status':'PASS','prepare_manifest_sha256':sha(PREP_MANIFEST),'prepare_outcome_sha256':sha(PREP_OUTCOME),'state_sha256':ssha,'readiness_sha256':rsha,'artifact_sha256':{rel(p):sha(p) for p in artifacts},'fixtures_passed':fx['passed'],'fixtures_total':fx['total'],'both_reviewer_manifests_frozen_before_first_provider_call':True,'provider_called':False,'next_authorized_stage':EXEC_A};write_once(PREP_RECEIPT,receipt)
...
def verify_prepare():
 ...
 if all(checks.values()):
  ...
  checks.update({'adapter':pm.get('adapter_sha256')==sha(SELF),'40_240':w.get('case_count')==40 and w.get('claim_count')==240 and sum(len(c['claims']) for c in w['cases'])==240,'blind':all(w.get(k) is False for k in ['evidence_present','support_labels_present','old_gold_present','arm_outputs_present','other_reviewer_output_present']),'fixtures':fx.get('status')=='PASS' and fx.get('passed')==fx.get('total')==11,'state_gate':s.get('next_authorized_stage')==EXEC_A,'readiness_gate':r.get('next_authorized_stage')==EXEC_A,'provider_not_called':s.get('successor_type_metadata_provider_called') is False,'confirmatory_closed':s.get('confirmatory_dataset_opened') is False,'runtime_unauthorized':s.get('runtime_integration_authorized') is False})
...
def exec_paths(r):
 if r=='a':return STATE_PREP,READY_PREP,PATTERN/'phase7_3_3_d_support_stage_state_v39.json',REPORTS/'phase7_3_3_d1_reference_construction_readiness_v50.json',EXEC_A
 return PATTERN/'phase7_3_3_d_support_stage_state_v39.json',REPORTS/'phase7_3_3_d1_reference_construction_readiness_v50.json',PATTERN/'phase7_3_3_d_support_stage_state_v40.json',REPORTS/'phase7_3_3_d1_reference_construction_readiness_v51.json',EXEC_B
...
def finalize(r,status,nxt,subsha=None,resha=None):
 si,ri,so,ro,_=exec_paths(r);s=copy.deepcopy(load(si));q=copy.deepcopy(load(ri));sv=39 if r=='a' else 40;rv=50 if r=='a' else 51;line={}
 if subsha:line[f'multi_claim_successor_type_metadata_reviewer_{r}_submission_v1_sha256']=subsha
 if resha:line[f'multi_claim_successor_type_metadata_reviewer_{r}_execution_result_v1_sha256']=resha
 s.setdefault('artifact_lineage',{}).update(line);q.setdefault('artifact_lineage',{}).update(line);completed=status.endswith('completed')
 s.update({'schema_version':sv,'state_id':f'phase7.3.3-d-support-stage-state-v{sv}','status':status,'next_authorized_stage':nxt,'successor_type_metadata_provider_called':True,f'successor_type_metadata_reviewer_{r}_completed':completed,'successor_type_metadata_review_completed':r=='b' and completed,'confirmatory_opening_authorized':False,'confirmatory_dataset_opened':False,'runtime_integration_authorized':False})
...
def execute(r):
 ...
 for case in w['cases']:
  ...
  if cp.exists():
   ...
   rows=normalize(case,{'annotations':[{k:x[k] for k in ['claim_id','claim_role','claim_type']} for x in d['annotations']]},r);case_results.append(d['case_result']);allrows.extend(rows);models.add(d['provider_reported_model']);continue
  if seen(msha,r,cid):raise ValueError('authoritative_content_seen_without_checkpoint:'+cid)
  case_json={'case_id':cid,'candidate_text':case['candidate_text'],'claims':[{'claim_id':x['claim_id'],'source_excerpt':x['source_excerpt']} for x in case['claims']]};user=user_template.replace('{{CASE_JSON}}',json.dumps(case_json,ensure_ascii=False,indent=2))
  ...
  try:
   ...
   cpdoc={'schema_version':1,'checkpoint_id':f'phase7.3.3-d-multi-claim-successor-type-metadata-{r}-{cid}-v1','manifest_sha256':msha,'reviewer':r,'case_id':cid,'provider_reported_model':reported,'case_result':cr,'annotations':rows,'raw_provider_content_stored':False};write_once(cp,cpdoc);case_results.append(cr);allrows.extend(rows);models.add(reported)
...
def verify_reviewer(r):
 ...
 if all(checks.values()):
  ...
  for c in w['cases']:
   ...
   d=load(p)
   try:
    rows=normalize(c,{'annotations':[{k:x[k] for k in ['claim_id','claim_role','claim_type']} for x in d['annotations']]},r);replay=replay and rows==d['annotations'];ids.extend(x['claim_id'] for x in rows)
