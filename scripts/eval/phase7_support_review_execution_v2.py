...
PROTOCOL=CONFIG/'phase7_3_3_d_support_review_execution_protocol_v2.json';POLICY=CONFIG/'phase7_3_3_d_support_review_execution_policy_v2.json';PROMPT=CONFIG/'phase7_3_3_d_support_reviewer_prompt_v1.md';FIXTURES=REPORTS/'phase7_3_3_d_support_review_contract_fixtures_v2.json'
GOLD=DATA/'phase7_3_3_d_boundary_gold_v1.json';FREEZE=REPORTS/'phase7_3_3_d_boundary_gold_freeze_receipt_v1.json';STATE=DATA/'phase7_3_3_d_support_stage_state_v5.json';READY=REPORTS/'phase7_3_3_d1_reference_construction_readiness_v16.json';CORRECTION=REPORTS/'phase7_3_3_d_support_lineage_correction_receipt_v1.json';AUDIT=REPORTS/'phase7_3_3_d_support_request_configuration_audit_v1.json'
BASE='https://api.gpt.ge/v1';CRED='PHASE7_ATOMIC_JUDGE_API_KEY';TEMP=0;TOP_P=1;MAX_TOKENS=16000;TIMEOUT=600;RF={'type':'json_object'}
...
def checkpoint(r,c):return REPORTS/f'phase7_3_3_d_support_reviewer_{r}_cases_v2'/f'{c}.json'
...
def negative(r):return REPORTS/f'phase7_3_3_d_support_reviewer_{r}_negative_result_v2.json'
...
def fixtures_doc():
 ...
 return {'schema_version':1,'report_id':'phase7.3.3-d1-b-support-review-contract-fixtures-v2','fixture_count':len(rs),'fixtures_passed':sum(x['fixture_passed'] for x in rs),'all_fixtures_passed':all(x['fixture_passed'] for x in rs),'provider_called':False,'held_out_accessed':False,'results':rs}
def entry_gate():
 req=[GOLD,FREEZE,STATE,READY,CORRECTION,AUDIT]+[packet(r) for r in REVIEWERS]+[template(r) for r in REVIEWERS];missing=[str(x.relative_to(ROOT)) for x in req if not x.exists()]
 if missing:raise ValueError(f'missing:{missing}')
 s=load(STATE);rd=load(READY);cr=load(CORRECTION);au=load(AUDIT);checks={'state_v5':s.get('schema_version')==5,'gold_hash':s.get('boundary_gold_sha256')==sha(GOLD),'receipt_hash':s.get('boundary_gold_freeze_receipt_sha256')==sha(FREEZE),'correction':cr.get('status')=='completed_support_lineage_correction','v1_terminal':s.get('support_execution_v1',{}).get('same_version_retry_allowed') is False and s.get('support_reviewer_a_completed') is False and s.get('support_reviewer_b_completed') is False,'v2_authorized':s.get('next_authorized_stage')=='new_support_execution_version_after_credential_remediation' and rd.get('next_authorized_stage')=='freeze_new_support_execution_version_after_valid_gateway_credential_is_available','request_audit':au.get('status')=='confirmed_adapter_credential_routing_defect' and au.get('authorized_remediation',{}).get('new_execution_version')=='v2' and au.get('authorized_remediation',{}).get('to')==CRED,'not_frozen':s.get('support_gold_frozen') is False and rd.get('support_gold_frozen') is False,'claims_118':s.get('boundary_claim_count')==118,'held_out_false':s.get('held_out_accessed') is False and rd.get('held_out_accessed') is False}
 for r in REVIEWERS:
...
def verify_execution(r):
 ...
 if done:
  p=load(packet(r));s=load(submission(r));res=load(result(r));ids=[x['boundary_claim_id'] for c in p['cases'] for x in c['boundary_claims']];checks.update({'submission_completed':s.get('completed') is True,'decisions_118':len(s.get('claims',[]))==118,'ids_exact':[x.get('boundary_claim_id') for x in s['claims']]==ids,'result_completed':res.get('status')=='completed','result_hash':res.get('submission_sha256')==sha(submission(r)),'checkpoints_10':len(list((REPORTS/f'phase7_3_3_d_support_reviewer_{r}_cases_v2').glob('*.json')))==10,'held_out_false':s.get('held_out_accessed') is False and res.get('held_out_accessed') is False})
 if neg:
